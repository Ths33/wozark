...
```
Ruth (Rust/Axum)       Wendy (TypeScript/Fastify)      Marty (React/Vite)       Jonah (planned)
Sensor                 Brain                            Dashboard                Analyst
├─ METAR 3s poll       ├─ Threshold detection           ├─ Station cards          ├─ AI daily max prediction
├─ PWS 60s poll        ├─ Trading guards                ├─ Positions + P&L        ├─ RAG + historical patterns
└─ POST raw → Wendy    ├─ CLOB execution (buy/sell)     ├─ Logs + trace timeline  └─ Strategic entry signal
...
- `wbot-ruth/.env` — WENDY_URL, RUTH_SECRET, WU_API_KEY
